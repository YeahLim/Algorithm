# 진홍이는 숫자를 좋아한다. 
# 오늘도 숫자를 가지고 놀던 진홍이는 두 숫자의 비트 우정지수를 구해보았다. 
...
# 예를 들어, 10진수 11과 12의 비트 우정지수를 구해보자. 
# 11을 이진수로 나타내면 1011이고, 12를 이진수로 나타내면 1100이다. 
...
import sys
n = int(input())
binary = [sys.stdin.readline() for _ in range(n)]
for b in binary:
    fir = b.split()[0]
    sec = b.split()[1]
